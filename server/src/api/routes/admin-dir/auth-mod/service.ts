...
async function requestToken(code: string): Promise<{ token: string; expiration: Date }> {
	try {
		const payload = {
...
async function refreshToken(code: string): Promise<{ token: string; expiration: Date }> {
	try {
		// Decrypt the token
